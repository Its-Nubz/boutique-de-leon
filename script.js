...
const resetReviewDialog=()=>{const form=document.getElementById('reviewForm');const success=document.getElementById('reviewSuccess');form.hidden=false;success.hidden=true};document.querySelector('.review-close').addEventListener('click',()=>{reviewDialog.close();resetReviewDialog()});document.querySelector('.review-success-close').addEventListener('click',()=>{reviewDialog.close();resetReviewDialog()});
reviewDialog.addEventListener('click',e=>{if(e.target===reviewDialog)reviewDialog.close()});

function setupForm(form){form.addEventListener('submit',async e=>{e.preventDefault();if(!form.reportValidity())return;const name=form.querySelector('[name="name"]');const phone=form.querySelector('[name="phone"]');const email=form.querySelector('[name="email"]');if(!name.value.trim()){name.setCustomValidity('Please enter your name.');name.reportValidity();name.setCustomValidity('');return}if(phone){const digits=phone.value.replace(/\D/g,'');if(digits.length!==10){phone.setCustomValidity('Please enter a 10-digit phone number.');phone.reportValidity();phone.setCustomValidity('');return}phone.value=digits}if(email&&!email.value.trim()){email.setCustomValidity('Please enter your email address.');email.reportValidity();email.setCustomValidity('');return}const button=form.querySelector('button[type="submit"]');const status=form.querySelector('.form-status');const original=button.textContent;button.disabled=true;button.textContent='Sending...';status.textContent='';try{const response=await fetch(form.action,{method:'POST',body:new FormData(form),headers:{Accept:'application/json'}});if(response.ok){if(form.id==='reviewForm'){form.reset();form.hidden=true;const success=document.getElementById('reviewSuccess');success.hidden=false;success.querySelector('.review-success-close').focus()}else{status.textContent='Thank you! Your message has been sent to Boutique De Leon.';form.reset();if(form.id==='inquiryForm')setTimeout(()=>dialog.close(),1800)}}else status.textContent="We couldn't send your message. Please try again."}catch(error){status.textContent="We couldn't send your message. Please try again."}finally{button.disabled=false;button.textContent=original}})}
